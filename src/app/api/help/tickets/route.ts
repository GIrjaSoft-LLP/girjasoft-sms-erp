...
export async function GET() {
  try {
    ...
    const items = await SupportTicket.find(scopedQuery(ctx.workspaceId)).sort({ createdAt: -1 }).limit(200).lean();
    const counts = {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await SupportTicket.create({
      ...
      createdByRole: ctx.session.roleSlugs?.[0] ?? ctx.session.sessionRole,
      type,
      subject,
      description,
      priority,
